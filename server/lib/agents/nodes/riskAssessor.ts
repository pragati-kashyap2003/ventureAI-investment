...
export async function riskAssessorNode(
  ...
  const { researchData, financialData, onProgress } = state;

  onProgress?.({
    node: "riskAssessor",
    status: "active",
    message: `⚠️ Assessing market, competitive, and regulatory risks for ${companyName}...`,
  });
  ...
  return createStructuredStateUpdate(state, { riskData, researchData, financialData });
}
