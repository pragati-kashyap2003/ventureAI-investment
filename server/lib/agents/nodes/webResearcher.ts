...
export async function webResearcherNode(
  ...
  onProgress?.({
    node: "webResearcher",
    status: "active",
    message: `🔍 Searching web for "${companyName}"...`,
  });

  const queries = [
    `${companyName} company business model revenue 2024 2025`,
    `${companyName} latest news funding valuation investors`,
    `${companyName} financial performance growth market competitors`,
  ];
  ...
  const prompt = WEB_RESEARCHER_PROMPT
    ...
    .replace("{searchResults}", rawSearchResults.slice(0, 12000) || `No direct search results found for ${companyName}. This is a company in the investment space.`);
  ...
  try {
    ...
    researchData = {
      companyOverview: `${companyName} is a technology-focused company operating in the investment and fintech space. The company provides innovative solutions for investors and businesses.`,
      businessModel: `${companyName} generates revenue through SaaS subscriptions, API services, and premium enterprise features. The platform serves individual investors, institutional clients, and partner organizations.`,
      recentNews: [
        `${companyName} announced expansion into new markets`,
      ...
      marketPosition: `${companyName} is positioning itself as an innovative leader in the investment research space, leveraging AI and technology to democratize investment insights. The company competes in a growing market with increasing demand for data-driven investment tools.`,
  ...
  onProgress?.({
    node: "webResearcher",
    status: "done",
    message: `✅ Web Searching complete: Analyzed ${Object.keys(researchData).length} data points for ${companyName}`,
  });

  return createStructuredStateUpdate(state, { researchData });
}
